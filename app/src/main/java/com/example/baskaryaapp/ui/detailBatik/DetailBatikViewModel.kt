...
class DetailBatikViewModel(private val bookmarkBatikRepository: BookmarkBatikRepository) : ViewModel() {
    ...
    fun saveBatik(bookmarkBatik: BookmarkBatik) {
        CoroutineScope(Dispatchers.IO).launch {
            bookmarkBatikRepository.setFavoriteUser(bookmarkBatik)
        }
    }

    fun deleteBatik(id: Int) {
        CoroutineScope(Dispatchers.IO).launch {
            bookmarkBatikRepository.deleteFavoriteUser(id)
        }
    }

    suspend fun isBookmarked(id: Int) = bookmarkBatikRepository.isBookmarked(id)
}